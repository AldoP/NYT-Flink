...
public class SocketWindowWordCount {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Tuple3<String, CommentLog, Integer>> my_sum = timestampedAndWatermarked
                .keyBy(value -> value.f0.articleID)
                .timeWindow(Time.minutes(60))
                .sum(1)
                .map(new MapFunction<Tuple2<CommentLog, Integer>, Tuple3<String, CommentLog, Integer>>() {
                    @Override
                    public Tuple3<String, CommentLog, Integer> map(Tuple2<CommentLog, Integer> logIntegerTuple2) throws Exception {
        ...
        DataStream<Tuple2<CommentLog, Integer>> resultWind = my_sum
                .keyBy(0)
                .timeWindow(Time.minutes(60))
                .apply(new WindowFunction<Tuple3<String, CommentLog, Integer>, Tuple2<CommentLog, Integer>, Tuple, TimeWindow>() {
                    @Override
                    public void apply(Tuple tuple, TimeWindow timeWindow,
                                      Iterable<Tuple3<String, CommentLog, Integer>> iterable,
                                      Collector<Tuple2<CommentLog, Integer>> collector) throws Exception {

                        List<Tuple2<Integer, CommentLog>> tuple2s = new ArrayList<Tuple2<Integer, CommentLog>>();

                        for(Tuple3<String, CommentLog, Integer> my_tuple : iterable){
                            tuple2s.add( new Tuple2<Integer, CommentLog> (my_tuple.f2, my_tuple.f1));
                        }
